...
function calculateAverage(pressureThreshold) {
    const filteredObjects = weather.filter((city) => {
        return city.pressure >= pressureThreshold
    })
    const arrayOfTemperature = filteredObjects.map((w) => {
        return w.temperature
    })
    let average = arrayOfTemperature.reduce((acc, temperature) => acc + temperature, 0)

    return (average / filteredObjects.length).toFixed(1)
